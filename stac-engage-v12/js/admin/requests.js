...
function confirmDenyRequest(id) {
  ...
  updateRequestBadge();
  // TODO (Supabase): await db.from('event_requests').update({ status: 'denied', admin_note: note }).eq('id', id);
  // await db.from('notifications').insert({ user_id: r.user_id, text: `Your event request was denied. ${note}` });
}
