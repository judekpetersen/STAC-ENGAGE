...
function C_eventRowV7(e) {
  const isFull     = WAITLIST_EVENTS[e.id];
  const onWaitlist = waitlistState[e.id];
  const needsFeedback = FEEDBACK_EVENTS[e.id] && !feedbackState[e.id];
...
function renderFeedbackSection() {
  const needsFeedback = DATA.events.filter(e => FEEDBACK_EVENTS[e.id] && !feedbackState[e.id]);
        ...
        <div class="modal-body">
          ...
          <div class="field-row-2">
            ...
            <div class="field"><label>Organization</label>
              <select id="er-org">
                <option value="">Select your org</option>
                ${DATA.orgs.map(o => `<option value="${o.name}">${o.name}</option>`).join('')}
                <option value="Independent">Independent / No org</option>
              </select>
            </div>
          </div>
          <div class="field-row-2">
...
function submitEventRequest() {
  ...
  // Refresh events tab to show pending request
  const content = document.getElementById('app-content');
  if (content) content.innerHTML = renderEvents();

  // TODO (Supabase): await db.from('event_requests').insert({ ...newReq, user_id: currentUser.id });
  // Then insert notification for admins
}

/* Patch renderEvents to add the request button and my requests section */
const _origRenderEvents = renderEvents;
window.renderEvents = function() {
  const myRequests = EVENT_REQUESTS.filter(r => r.submittedBy === 'Jude M.');
  const base = _origRenderEvents();

  // Inject request button and my requests panel after the events card
  const requestSection = `
    <div class="card" style="margin-bottom:1rem;">
      <div class="card-head">
        <div class="card-title"><i class="ti ti-calendar-plus"></i>Request a campus event</div>
  ...
  // Insert before the feedback card (last card)
  return base.replace(
    '<div class="card">\n      <div class="card-head">\n        <div class="card-title"><i class="ti ti-star"></i>Leave feedback</div>',
    requestSection + '\n    <div class="card">\n      <div class="card-head">\n        <div class="card-title"><i class="ti ti-star"></i>Leave feedback</div>'
  );
